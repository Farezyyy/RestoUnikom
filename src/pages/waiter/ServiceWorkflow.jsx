  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Active Service Workflow</h2>
          <p className="text-xs text-gray-500 mt-0.5">Real-time dish tracking & serving lifecycle from Kitchen to Table</p>
        </div>
        <div className="flex items-center space-x-4 text-xs font-semibold">
          <div className="flex items-center text-orange-600 bg-orange-50 px-2.5 py-1 rounded-md border border-orange-200">
            <span className="w-2 h-2 rounded-full bg-orange-500 mr-2 animate-pulse"></span> Dimasak (Cooking)
          </div>
          <div className="flex items-center text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md border border-blue-200">
            <span className="w-2 h-2 rounded-full bg-blue-500 mr-2 animate-pulse"></span> Siap Disajikan (Ready)
          </div>
          <div className="flex items-center text-green-600 bg-green-50 px-2.5 py-1 rounded-md border border-green-200">
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <h3 className="text-base font-bold text-gray-700">No Active Service In Progress</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map(service => (
            <div key={service.courseId} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
              <div className={`p-4 border-b flex justify-between items-center ${
                service.status === 'SIAP' ? 'bg-blue-50/70 border-blue-100' : 
                service.status === 'DIMASAK' ? 'bg-orange-50/70 border-orange-100' : 
                service.status === 'DISAJIKAN' ? 'bg-green-50/70 border-green-100' : 'bg-gray-50 border-gray-100'
              }`}>
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900 text-base">Meja {service.tableNo}</h3>
                    <p className="text-xs text-gray-500 font-bold flex items-center mt-0.5">
                ...
                <div>
                  <span className={`text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full ${
                    service.status === 'SIAP' ? 'text-blue-700 bg-blue-100 border border-blue-200' : 
                    service.status === 'DIMASAK' ? 'text-orange-700 bg-orange-100 border border-orange-200' : 
                    service.status === 'DISAJIKAN' ? 'text-green-700 bg-green-100 border border-green-200' : 'text-gray-600 bg-gray-100'
                  }`}>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 mb-3 space-y-1 text-xs">
                    <div className="flex justify-between items-center text-gray-600 font-semibold">
                      <span className="flex items-center">
                        ...
                        Total Guests: <strong className="text-gray-900 ml-1">{service.totalGuests} Pax</strong>
                      </span>
                  ...
                  <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Fase Course</span>
                  <p className="text-base font-bold text-primary">{service.course}</p>
                  <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <p className="text-sm font-semibold text-gray-800 flex justify-between">
                      <span>{service.menuName}</span>
                      <span className="text-gray-500">x{service.qty}</span>
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {service.status === 'DISAJIKAN' && (
                    <div className="w-full py-2 bg-green-50 text-green-700 rounded-lg text-xs font-bold flex items-center justify-center space-x-1.5 border border-green-200">
                      <CheckCircle2 className="w-3.5 h-3.5 text-green-600" />
                      <span>Hidangan Sudah Disajikan</span>
                    </div>
                  )}
                  {service.status === 'DIMASAK' && (
                    <div className="w-full py-2 bg-orange-50 text-orange-700 rounded-lg text-xs font-bold flex items-center justify-center space-x-1.5 border border-orange-200">
                      <ChefHat className="w-3.5 h-3.5 text-orange-600 animate-pulse" />
                      <span>Sedang Dimasak Koki di Dapur...</span>
                    </div>
                  )}
                  {service.status === 'MENUNGGU' && (
                    <div className="w-full py-2 bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold flex items-center justify-center space-x-1.5 border border-gray-200">
