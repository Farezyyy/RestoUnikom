  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="flex-1 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
            <div>
              <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center">
                <Utensils className="w-5 h-5 mr-2 text-primary dark:text-accent" /> Additional Orders & Beverages
              </h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">Live database items for active tables</p>
            </div>
        ...
        <div className="p-6 flex-1 overflow-auto bg-gray-50/30 dark:bg-gray-800/30">
          {loading ? (
            <div className="p-12 text-center text-gray-500 dark:text-gray-400 font-medium">Loading live menu items...</div>
          ) : filteredMenu.length === 0 ? (
            <div className="p-12 text-center text-gray-400 dark:text-gray-400 italic">No additional menu items found.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredMenu.map(item => (
                <div
                  key={item.id}
                  className={`p-4 rounded-xl border transition-all ${item.aktif !== false ? 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 hover:border-primary/50 cursor-pointer shadow-sm' : 'bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 opacity-60'}`}
                  onClick={() => addToCart(item)}
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="pr-2">
                      <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-extrabold mb-1">{item.kategori || 'A La Carte'}</p>
                      <h4 className="font-bold text-gray-900 dark:text-gray-100">{item.nama}</h4>
                    </div>
                    {item.aktif === false ? (
                      <span className="flex items-center text-xs text-red-500 dark:text-red-400 font-medium bg-red-50 dark:bg-red-900/40 px-2 py-1 rounded">
                        <AlertCircle className="w-3 h-3 mr-1" /> Unavailable
                      </span>
                    ) : (
                      <span className="text-sm font-extrabold text-primary dark:text-accent">Rp {Number(item.harga).toLocaleString()}</span>
                    )}
                  </div>
                </div>
              ))}
