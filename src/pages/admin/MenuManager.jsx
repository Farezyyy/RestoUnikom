...
export default function MenuManager() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Loading Menu from Database...</div>;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-[calc(100vh-8rem)]">
      <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-xl font-serif text-gray-800">Menu & Recipe Management</h2>
          <p className="text-sm text-gray-500 mt-1">Manage dishes, prices, and recipe links</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {filtered.map(item => {
              ...
              return (
                <tr key={item.id} className={`border-b border-gray-100 transition-colors ${isAvailable ? 'hover:bg-gray-50' : 'bg-red-50/30'}`}>
                  <td className="p-4 font-bold text-gray-800">{item.name}</td>
                  <td className="p-4 text-gray-600">
                    <span className="px-2 py-1 bg-gray-100 border border-gray-200 rounded text-xs tracking-wider uppercase font-medium">{item.category}</span>
                  </td>
                  <td className="p-4">
                    {editingId === item.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          className="w-32 p-1 border border-primary rounded text-sm focus:outline-none"
                          value={editPrice}
                          onChange={(e) => setEditPrice(e.target.value)}
                          autoFocus
                        />
                        <button onClick={() => saveEdit(item.id)} className="p-1 bg-green-100 text-green-700 rounded"><Check className="w-4 h-4" /></button>
                        <button onClick={() => setEditingId(null)} className="p-1 bg-red-100 text-red-700 rounded"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                  ...
                  <td className="p-4">
                    <div className="flex items-center">
                      ...
                      {!isAvailable && (
                        <div className="ml-3 text-xs text-red-600 font-medium flex items-center group relative cursor-help">
                          <AlertCircle className="w-4 h-4 mr-1" /> Sold Out
                          <HelpCircle className="w-3 h-3 ml-1 text-gray-400" />

                          <div className="absolute left-full ml-2 w-max px-3 py-2 bg-gray-900 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                            {availability.reason}
                          </div>
                        </div>
                      )}
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="p-2 text-indigo-600 hover:bg-indigo-50 rounded transition-colors mr-2"
                      title="Manage Recipe"
                    >
                    ...
                    <button
                      ...
                      className="p-2 text-blue-600 hover:bg-blue-50 rounded transition-colors disabled:opacity-50 mr-2"
                      title="Edit Price"
                    >
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan="5" className="p-8 text-center text-gray-500 italic">No menu items found.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-xl font-serif text-gray-800">Add New Menu Item</h3>
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" className="w-full p-2 border border-gray-200 rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" value={newMenu.nama} onChange={e => setNewMenu({ ...newMenu, nama: e.target.value })} placeholder="e.g. Wagyu Steak" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Category</label>
                <select required className="w-full p-2 border border-gray-200 rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" value={newMenu.kategori} onChange={e => setNewMenu({ ...newMenu, kategori: e.target.value })}>
              ...
              <div>
                ...
                <input required type="number" min="0" className="w-full p-2 border border-gray-200 rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" value={newMenu.harga} onChange={e => setNewMenu({ ...newMenu, harga: e.target.value })} />
              </div>
