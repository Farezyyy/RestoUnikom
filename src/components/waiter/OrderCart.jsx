...
export default function OrderCart({
  ...
  return (
    <div className="w-full lg:w-96 bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col">
      <div className="p-6 border-b border-gray-100 bg-gray-50/50">
        <h3 className="text-lg font-bold text-gray-800 flex items-center">
          <ShoppingCart className="w-5 h-5 mr-2 text-primary" /> {title}
        </h3>
        
        <div className="mt-4">
          <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Target Table</label>
          {hasActiveTables ? (
            <select 
              className="w-full p-2.5 border border-gray-200 rounded-lg text-sm font-bold bg-white focus:outline-none focus:border-primary"
              value={selectedTable || ''}
            ...
            <div className="p-2.5 bg-orange-50 border border-orange-200 rounded-lg text-xs font-bold text-orange-700 flex items-center">
            ...
            {cart.map(item => (
              <div key={item.id} className="flex justify-between items-center pb-4 border-b border-gray-100 last:border-0">
                <div className="flex-1 pr-4">
                  <p className="text-sm font-bold text-gray-900 line-clamp-1">{item.name || item.nama}</p>
                ...
                <div className="flex items-center space-x-3 bg-gray-50 rounded-lg p-1 border border-gray-100">
                  <button 
                    ...
                    className="p-1 text-gray-500 hover:text-primary transition-colors bg-white rounded shadow-sm border border-gray-200 cursor-pointer"
                  >
      ...
      <div className="p-6 border-t border-gray-100 bg-gray-50/50">
        {totalCart > 0 && (
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xl font-extrabold text-gray-900">Rp {Number(totalCart).toLocaleString()}</span>
          </div>
        )}
        <button 
          onClick={onCheckout}
