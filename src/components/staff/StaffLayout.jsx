...
import { Bell, LogOut, Clock, AlertTriangle, CheckCircle, Info, Check, Trash2, X } from 'lucide-react';
import api from '../../api';
    ...
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-xl font-serif font-bold text-primary">Resto Unikom</h2>
        ...
        <div className="p-4 border-t border-gray-200">
          <button
            ...
            className="flex items-center space-x-3 px-4 py-3 w-full text-left text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
          >
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 z-30">
          <div className="flex items-center gap-6">
            ...
            <div className="hidden md:flex items-center gap-3 border-l border-gray-200 pl-6">
              <div className={`text-xs font-bold px-2 py-1 rounded ${currentSession === 'Off-Session' ? 'bg-gray-100 text-gray-500' : 'bg-green-100 text-green-700'}`}>
                {currentSession}
              </div>
              <div className={`text-xs font-bold px-2 py-1 rounded border ${sessionCountdown.isPreEndWarning ? 'bg-red-50 text-red-700 border-red-200 animate-pulse' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
          ...
          <div className="flex items-center justify-center space-x-4">
            ...
            <div className="relative">
              <button 
                ...
                className="p-2 relative text-gray-500 hover:text-primary transition-colors cursor-pointer rounded-full hover:bg-gray-100"
                title="Notifications"
              >
              ...
              {showNotifDropdown && (
                <div className="absolute right-0 mt-2 w-96 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <div className="flex items-center space-x-2">
                      <h3 className="font-bold text-gray-900 text-sm">Notifications</h3>
                      {unreadNotifs.length > 0 && (
                        <span className="px-2 py-0.5 bg-red-100 text-red-700 text-xs font-extrabold rounded-full">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        <div 
                          ...
                          className={`p-3.5 flex items-start space-x-3 transition-colors cursor-pointer ${n.dibaca ? 'bg-white text-gray-600' : 'bg-blue-50/60 font-semibold text-gray-900'}`}
                        >
                          ...
                          <div className="flex-1 text-xs">
                            <div className="flex justify-between items-center">
                              <p className="font-bold text-gray-900">{n.judul}</p>
                              <span className="text-[10px] text-gray-400 font-mono">
                                {new Date(n.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </span>
                            </div>
                            <p className="text-gray-600 mt-0.5 leading-relaxed">{n.pesan}</p>
                          </div>
